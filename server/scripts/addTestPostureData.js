// Script to add test posture data for a specific pig and date range
...
async function generatePostureData(pigId, startDateStr, endDateStr, recordsPerDay = 48) {
  try {
    // Parse dates
    ...
    // Check if pig exists
    const pig = await Pig.findOne({ pigId: parseInt(pigId) });
    if (!pig) {
...
if (args.length < 3) {
  console.log('Usage: node addTestPostureData.js <pigId> <startDate> <endDate>');
  console.log('Example: node addTestPostureData.js 129 2022-07-21 2022-08-02');
  process.exit(1);
}
...
// Run the function
generatePostureData(pigId, startDate, endDate)
  .then(count => {
